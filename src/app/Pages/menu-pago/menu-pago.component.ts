...
export class MenuPagoComponent implements OnInit {
  ...
  validar() {
    if (this.cardForm.valid) {
      ...
      this.correcto("Se realizó el pago correctamente.");
  ...
  pagar() {
    if (this.tarjetaValor) {
      ...
      const asientoPromises = this.asientosSeleccionados.map((idAsiento) => {
        return this.boletoService.registrarBoleto({
          idBoleto: 0,
      ...
      Promise.all(asientoPromises)
        .then(() => {
          console.log('Boleto registrado correctamente.');
          this.correcto("Pago realizado con éxito");

          // Actualizar estado de los asientos a OCUPADO
          const asientoUpdatePromises = this.asientosSeleccionados.map((idAsiento) =>
            this.asientoService.actualizarEstadoAsiento(parseInt(idAsiento), 2).toPromise()
          );

          return Promise.all(asientoUpdatePromises);
        })
        .then(() => {
