  ...
  buscarRutas() {
    ...
    this.rutaService.buscarRutaPorTerminales(this.selectedTerminalOrigen, this.selectedTerminalDestino).subscribe((data: Ruta) => {
        ...
        if (isNaN(fechaSalida.getTime())) {
            console.error('Fecha de salida inválida:', this.selectedFechaSalida);
            return;
        }

        console.log('Buscando viajes con:', fechaSalida.toISOString(), this.idRuta);
        this.viajeService.buscarViajes(fechaSalida, this.idRuta).subscribe((data: Viaje[]) => {
            console.log('Datos de viajes obtenidos:', data);
            if (data.length === 0) {
                this.validatorService.tarjeta("No se encontraron viajes disponibles para esta ruta", false);
                return;
            }
